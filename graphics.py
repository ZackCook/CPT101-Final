...
class Circle(Oval):
    ...
    #Created by zack cook 10/22/18
    def getCenterX(self):
        return self.getCenter().getX()
...
class Text(GraphicsObject):
    ...
    def _draw(self, canvas, options):
        p = self.anchor
        x,y = canvas.toScreen(p.x,p.y)
        self.drawn = True#added by zack cook
        return canvas.create_text(x,y,options)

    def isDrawn(self):#added by zack cook
        return self.drawn

    def setXVelocity(self, vel):#added by zack cook
        self.xVelocity = vel

    def setYVelocity(self,vel):#added by zack cook
        self.yVelocity = vel

    def getXVelocity(self):#added by zack cook
        return self.xVelocity

    def getYVelocity(self):#added by zack cook
